...
import { AuthorizationWithKeyParams } from 'interfaces'
...
export class GoogleAuthorizationWithJson {
  private readonly config: AuthorizationWithKeyParams
	
  constructor(config: AuthorizationWithKeyParams) {
    this.config = config
  }
  ...
  private async buildKeysFile(config: AuthorizationWithKeyParams) {
    const fileData = `{
